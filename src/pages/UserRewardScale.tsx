...
import { currentCycleUp, rewardTiers } from '../data/rewardTiers'

function getNextTier() {
  return rewardTiers.find((tier) => tier.requiredUp > currentCycleUp) ?? rewardTiers[rewardTiers.length - 1]
}

export default function UserRewardScale() {
  const maxUp = rewardTiers[rewardTiers.length - 1].requiredUp
  const nextTier = getNextTier()
  const missingUp = Math.max(0, nextTier.requiredUp - currentCycleUp)
